...
def validate():
    ...
    try:
        # Request all rows with exact count
        resp = supabase.table("churn_data").select("*", count="exact").limit(1).execute()
        # supabase client may return count in resp.count or resp.get("count")
        supabase_count = None
        if hasattr(resp, "count") and resp.count is not None:
            supabase_count = int(resp.count)
        else:
            # fallback: try length of data returned via count property in response
            try:
                supabase_count = int(resp.get("count"))
            except Exception:
                # as last resort, fetch all rows (can be slow) and count
        ...
        print("⚠️ Could not fetch count from Supabase:", e)
        supabase_count = None
    ...
    contract_codes_ok = contract_codes_present.issubset({-1,0,1,2})  # allow -1 for unknown if used
