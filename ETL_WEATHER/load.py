...
def create_table_if_not_exists():
    ...
    try:
        ...
        print(" Please run the following SQL in your Supabase SQL editor if table doesn't exist:")
    ...
    for i in range(0, total, batch_size):
        ...
        try:
            ...
            if hasattr(res, "error") and res.error:
                print(f"  Batch {i//batch_size + 1} error: {res.error}")
            else:
                end = min(i + batch_size, total)
                print(f" Inserted rows {i+1}-{end} of {total}")
        except Exception as e:
            print(f"  Exception while inserting batch {i//batch_size + 1}: {e}")
